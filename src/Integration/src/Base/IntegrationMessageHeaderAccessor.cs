...
namespace Steeltoe.Integration
{
    public class IntegrationMessageHeaderAccessor : MessageHeaderAccessor
    {
        ...
        {
            int? sequenceNumber = (int?)GetHeader(SEQUENCE_NUMBER);
            return sequenceNumber ?? 0;
        }

        public int GetSequenceSize()
        {
            int? sequenceSize = (int?)GetHeader(SEQUENCE_SIZE);
        ...
        {
            if (this.readOnlyHeaders.Count == 0)
            {
            ...
            else
            {
                ...
                foreach (string header in this.readOnlyHeaders)
                {
